    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <p
                className={clsx(
                  'min-w-0 text-sm leading-7 text-slate-300',
        ...
        <div className="mt-auto pt-5">
          <div className="flex items-center justify-between gap-3 border-t border-white/10 pt-4 text-xs uppercase tracking-[0.18em] text-slate-400">
            <div className="flex min-w-0 flex-wrap items-center gap-3">
              <span className="sm:hidden">{submission.vote_count} votes</span>
              <span>{formatDate(submission.created_at)}</span>
              {submission.discord_handle ? <span className="truncate text-slate-500">{submission.discord_handle}</span> : null}
            </div>
            <span className="font-semibold text-slate-500 transition-colors duration-300 group-hover:text-cyan">Open project</span>
          </div>
        </div>
      </div>
    </Link>
